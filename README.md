...
- 站点、上游、同步任务、用户资料、通知和系统配置均可再次编辑；连接类配置保存前会重新验证，失败时保留原设置。
- 管理后台支持检查 GitHub 正式发行版并一键更新 Windows/Linux amd64 独立程序，下载内容必须通过 SHA-256 校验。
...
系统管理员可以在独立管理后台的“在线更新”页面检查并安装 GitHub 最新正式发行版。更新程序只接受与当前操作系统和架构匹配的文件，并在 SHA-256 校验通过后替换程序、保留 `.previous` 旧版本并自动重启。

在线更新适用于官方 Windows/Linux amd64 独立二进制。Docker 部署请更新镜像并重新创建容器，不在容器内部覆盖程序。源码自行编译时版本号默认为 `dev`，不会启用在线覆盖。
